...
public class examples implements examplesInterface{
	...
	public String ffmpegVideoEncoding_AVI2MP4(int r, int b, int bt, int pass,
		...
		String avi2mp4= "/root/bin/ffmpeg -y -i /tmp/ffmpeg/inputVideo1.avi -r "+r+" -b "+b+"k -bt "+bt+" -vcodec libx264 -pass "+pass+" -minrate "+minrate+" -maxrate "+maxrate+" -bufsize "+bufsize+"k -keyint_min "+keyint_min+" -sc_threshold "+sc_threshold+" -i_qfactor "+i_qfactor+" -threads "+threads+" -an outputVideo1.mp4";
	...
	public String ffmpegVideoEncoding_AVI2FLV(int b, int ar, int ab, int h,
		...
		String avi2flv= "/root/bin/ffmpeg -y -i /tmp/ffmpeg/inputVideo2.avi -b:a "+b+"k -ar "+ar+" -ab "+ab+" -s "+h+"x"+w+"  -r "+r+" -vframes "+vframes+" -threads "+threads+" -t "+t+" -g "+g+" outputVideo2.flv";
	...
	public String ffmpegAudioEncoding_WAV2M4A(int h, int w, int qa, int ba,
		...
		String wav2m4a="/root/bin/ffmpeg -y -i /tmp/ffmpeg/inputAudio.wav -s "+h+"x"+w+" -strict experimental -c:a aac -q:a "+qa+" -b:a "+ba+"k -pass "+pass+" -ac "+ac+" -threads "+threads+" outputAudio.m4a";
